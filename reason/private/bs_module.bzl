...
load(":providers.bzl", "ReasonModuleInfo", "BsModuleInfo")
...
def _bs_module_impl(ctx):
    ...
    return [
        ...
        BsModuleInfo(
            name=ctx.label.name, deps=ctx.attr.deps, srcs=sources, outs=outputs)
    ]
...
bs_module = rule(
    attrs={
        "config":
        attr.label(
            allow_single_file=[BS_CONFIG_EXT],
            mandatory=True,
        ),
        "srcs":
        attr.label_list(
        ...
        attr.label(
            default=
            "@com_github_ostera_rules_reason//reason/toolchain:bs-platform",
